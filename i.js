...
async function generateWallets(howMany = 30) {
  const privKeys = [];
...
(async () => {
  console.clear();
  console.log(chalk.green.bold("Auto‑Claim Faucet Injective + Auto‑Send BOT (30 Wallet per Siklus)"));

  const provider = new ethers.JsonRpcProvider(INJECTIVE_RPC);

  while (true) {
    // 1. Generate 30 wallet baru tiap siklus
    await generateWallets(30);
